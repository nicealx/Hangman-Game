...
function errorInput(
  ...
  if (count < countMax) {
    ...
    partImg.src = `./assets/img/man/${man[count]}`;
...
function checkWin(keyboardDiv, answerText) {
  disableKeys(keyboardDiv);
  body.append(createModal(answerText, "Ты выиграл!", "modal__button-win"));
}
...
function checkAnswer(
  ...
  if (check.length > 0) {
    correctInput(chars, keyboardDiv, answerDiv, answer);
  } else {
    errorInput(keyboardDiv, counterDynamic, target, answer, gallowsDiv);
  }
}
